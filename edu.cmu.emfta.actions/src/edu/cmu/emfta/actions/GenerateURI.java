...
public class GenerateURI {
	 ...
	 * @param ftaTree
	 */
	public GenerateURI(Event ftaEvent) {
		System.out.println("[GenerateURI] constructor");
...
//	}
	...
	public void process() {
		...
		System.out.println("[GenerateURI] calling process ");

		System.out.println("[GenerateURI] topEvent default URI = " + topEvent.getURI());

		if (topEvent.getURI() == null || topEvent.getURI() == "") {
			topEvent.setURI(EcoreUtil.getURI(topEvent).toString());
		}
...
//		int n = 0;
		for (List<Event> l : allEvents) {
			for (Event e : l) {
				...
				System.out.println("[GenerateURI] event e default URI = " + e.getURI());

				if (e.getURI() == null || e.getURI() == "") {
					URI uri = EcoreUtil.getURI(e);
					e.setURI(uri.toString());
